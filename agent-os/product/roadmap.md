...
13. [ ] **Geographic Navigation & Routing** — Implement State Selector Header (horizontal scroll) and dynamic routes (`/br/[state]`) for SEO-friendly state filtering. `M`
14. [ ] **Organizer Submission Flow** — Create an "Area do Organizador" with a submission form (MVP: Google Forms integration) for manual race entry and approval. `XS`
15. [ ] **Coupons & Discounts Page** — Develop a page to display discount coupons (manual entry or scraped), filterable by state, to provide extra value to runners. `S`
