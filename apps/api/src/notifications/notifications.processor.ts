...
export class NotificationsProcessor extends WorkerHost {
    ...
    private async handleSendPushNotification(job: Job) {
        ...
        this.logger.log(`Processing push notification for event: ${eventTitle} (${eventState})`);
        ...
        try {
            ...
            this.logger.log(`Found ${subscriptions.length} subscriptions for state ${eventState}`);
            ...
            this.logger.log(`Notification summary for event ${eventId}: ${successCount} sent, ${failureCount} failed`);

        } catch (error) {
            this.logger.error(`Failed to process notification job for event ${eventId}`, error);
            throw error;
        }
    }
}
