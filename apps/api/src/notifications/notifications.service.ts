...
export class NotificationsService {
    ...
    async upsertSubscription(createSubscriptionDto: CreateSubscriptionDto) {
        const { endpoint, keys, statePreferences, userAgent } = createSubscriptionDto;
    ...
    async sendNotification(subscription: any, payload: any) {
        try {
            ...
            this.logger.error(`Error sending notification to ${subscription.endpoint}`, error);
            if (error.statusCode === 410) {
                // Subscription expired or gone, remove it
                await this.prisma.pushSubscription.delete({
                    where: { endpoint: subscription.endpoint },
                });
                this.logger.log(`Removed expired subscription: ${subscription.endpoint}`);
            }
            throw error;
        }
    }
