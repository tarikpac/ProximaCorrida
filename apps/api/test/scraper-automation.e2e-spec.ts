...
describe('Scraper Automation', () => {
    ...
    beforeEach(async () => {
        ...
        schedulerService = module.get<ScraperSchedulerService>(ScraperSchedulerService);
    ...
    describe('Scheduler', () => {
        it('should add a job to the queue when triggered', async () => {
            ...
            expect(queue.add).toHaveBeenCalledWith('runScraper', {}, expect.objectContaining({
                attempts: 3,
                backoff: {
                    type: 'exponential',
                    delay: 30000,
    ...
    describe('Processor', () => {
        it('should call ScraperService.scrapeEvents', async () => {
            mockScraperService.scrapeEvents.mockResolvedValue({ newEvents: 5, updatedEvents: 2 });
        ...
        it('should handle errors gracefully', async () => {
            mockScraperService.scrapeEvents.mockRejectedValue(new Error('Scraping failed'));
            const job = { id: '1', name: 'runScraper' } as any;
